...
#include <iomanip>
#include <vector>
#include "/Users/Frankie/ROOTmacros/tdrstyle.C"
...
//string mcFile = "~/cms/analysis/results/2010_Jul/16Jul_mc_met30_conv/test_mc_mixture.root";
//string mcFile = "/Users/Frankie/2010/DataRuns/MC_30-08-10/all_mc_mixture.root";
//string mcFile = "/Users/Frankie/2010/DataRuns/HCPUpdate/MC/15Aug/allMC.root";
//string mcFile = "/Users/Frankie/2010/DataRunsOctober/MC111010/all_mc_mixture.root";
string mcFile = "/Users/Frankie/2010/DataRunsOctober/MC301010/all_mc_mixture.root";

// Ref Sel (78/nb)
//string mcFile = "~/cms/analysis/results/2010_Jul/14Jul_mc/test_mc_mixture.root";
//string dataFile = "~/cms/analysis/results/2010_Jul/13Jul_data/test_data.root";
//string dataFile = "/Users/Frankie/2010/DataRuns/Data_1300nb/data_data.root";
string dataFile = "/Users/Frankie/2010/DataRunsOctober/Data2010AB/data_data.root";

// Ref Sel, MET>25 (77/nb)
//string mcFile = "~/cms/analysis/results/2010_Jul/12Jul_mc_e20_met25/mc_met25.root";
//string dataFile = "~/cms/analysis/results/2010_Jul/12Jul_data_met25_fix/test_data.root";
...
//double intlumi = 63.68e-3; //pb-1
//double intlumi = 16.46e-3; //pb-1
//double intlumi = 11.4e-3; //pb-1
...
void SetDrawMC(int val){ draw_mc = val; }
void SetDrawData(int val){ draw_data = val; }

void SetDataFile(string p){ dataFile = p; }
void SetMCFile(string p){ mcFile = p; }

void SetDataIntLumi(double p){ intlumi = p; }
...
int fit(string do_fit_user="fix",
  ...
  TFile *fdata;
  TFile *fmc;
  //  TFile fdata;("../test_data.root"); //data
  //  TFile fmc("../test_mc_mixture.root"); //mc
  //string dataFile = "../test_data.root";
  //  string mcFile = "~/cms/analysis/results/2010_May/10May_spring10/with_missLayer_cut/test_mc_mixture.root";
  //string mcFile = "~/cms/analysis/results/2010_Apr/15Apr_rev80/test_mc_mixture.root"; //TEMPORARY

  cout << "data file: " << dataFile << endl;
  cout << "mc   file: " << mcFile << endl;

  if (draw_data) fdata = new TFile( Form("%s",dataFile.c_str() ) );
  else           fdata = new TFile( Form("%s",mcFile.c_str() ) );
  if (draw_mc) fmc = new TFile(Form("%s", mcFile.c_str() ) );
  ...
  //  TCanvas *c1 = new TCanvas("c1","c1",980*1.5,1050*1.5);//980,700
  ...
  for (int j=2; j<6; j++){ 
    ...
    if(draw_mc) n_cand_mc_predicted[j] = mc_all->Integral(1,control_bin_max);
    n_cand_data_observed[j] = all->Integral(1,control_bin_max);



    if(debug) cout << "amtb 1: Rebin, before"<< endl;
    ...
    if(debug) cout << "amtb 1: Rebin, after" << endl;
    ...
    //    if(npad==2) npad=3; //do not draw on pad 2
    c1->cd(npad);
    //    gPad = (TPad*)c1->GetPad(npad);   
    npad++;
	...
	if( bin_height > highest ) highest = bin_height;
      ...
      /*
      // customize
      if( njet[j]=="0j"  && YMAX_0j > 0 )  all->GetYaxis()->SetRangeUser(0,YMAX_0j);
      if( njet[j]=="1j"  && YMAX_1j > 0 )  all->GetYaxis()->SetRangeUser(0,YMAX_1j);
      if( njet[j]=="2j"  && YMAX_2j > 0 )  all->GetYaxis()->SetRangeUser(0,YMAX_2j);
      if( njet[j]=="3j"  && YMAX_3j > 0 )  all->GetYaxis()->SetRangeUser(0,YMAX_3j);
      if( njet[j]=="4mj" && YMAX_4mj > 0 )  all->GetYaxis()->SetRangeUser(0,YMAX_4mj);
      */
      //      if(j+1==2) all->GetYaxis()->SetRangeUser(0,10); //1j
      ...
      */
      ...
      if( njet[j]=="allj"  && YMAX_allj > 0 )  all->GetYaxis()->SetRangeUser(0,YMAX_allj);
      if( njet[j]=="0j"  && YMAX_0j > 0 )  all->GetYaxis()->SetRangeUser(0,YMAX_0j);
      if( njet[j]=="1j"  && YMAX_1j > 0 )  all->GetYaxis()->SetRangeUser(0,YMAX_1j);
      if( njet[j]=="2j"  && YMAX_2j > 0 )  all->GetYaxis()->SetRangeUser(0,YMAX_2j);
      if( njet[j]=="3j"  && YMAX_3j > 0 )  all->GetYaxis()->SetRangeUser(0,YMAX_3j);
    ...
    if(debug) cout << "amtb: gPad->ls()"<< endl;

    //gPad->ls();
    
    if(debug) cout << "amtb: set margin"<< endl;
    ...
    if(debug) cout << "amtb: set offset"<< endl;

    all->GetXaxis()->SetTitleOffset(0.8); //on stat

    if(debug) cout << "amtb: set title size"<< endl;

    all->GetXaxis()->SetTitleSize(0.07);

    if(debug) cout << "amtb: draw" << endl;

    all->Draw("ae"); 
    //    all->Draw("ae hist");
    if(draw_mc){
      qcdcomp->Draw("ahist same"); //add
      //qcd->Draw("ae same"); //do not show error for QCD MC
    }
    all->Draw("ae same");
    all->Draw("axis same");

    if(debug) cout << "amtb: update gPad"<< endl;

    gPad->Update();

    if(debug) cout << "amtb 3: get stat box"<< endl;
  ...
  //  double coord[4] = {0.33,0.3,0.6,0.9};
  //double coord[4] = {0.7,0.4,0.99,0.99};
  ...
  if(!draw_data) leg->AddEntry(all,  "All MC events","LP"); //Line+Marker, E(error)
  else           leg->AddEntry(all,  "Data","LP");
  if(draw_mc){
  ...
  if(do_fit != "none"){
    ...
    /*
    leg->SetX1(coord[0]+0.);//move to right  0.3
    leg->SetX2(coord[2]+0.);  //0.3
    leg->SetY1(coord[1]+0.5);  //new
    */
  }
  ...
  //c1->cd(1); leg->Draw();   add_cms_label(); add_njet_label(5);
  //c1->cd(2); leg->Draw();   add_cms_label(); add_njet_label(0);
  c1->cd(1); leg->Draw();   add_cms_label(); add_njet_label(1);
  c1->cd(2); leg->Draw();   add_cms_label(); add_njet_label(2);
  c1->cd(3); leg->Draw();   add_cms_label(); add_njet_label(3);
  c1->cd(4); leg->Draw();   add_cms_label(); add_njet_label(4);
  ...
  printf("   3 jet:   %2d %10.1f  %10.1f  %6.1f %%\n", rebin[4], nqcd_actual_sig[4], n_extrap[4], (n_extrap[4]/nqcd_actual_sig[4]-1)*100 );
  printf(" >=4 jet:   %2d %10.1f  %10.1f  %6.1f %%\n", rebin[5], nqcd_actual_sig[5], n_extrap[5], (n_extrap[5]/nqcd_actual_sig[5]-1)*100 );
  cout << "-----------------------------------------------\n"<< endl;
  ...
  for(int k=2; k<6; ++k){    myfile << n_extrap[k]/nqcd_actual_sig[k]-1 << endl;  }
...
void add_njet_label(int nj){
  ...
  if     (nj==0)  nj_label = "N_{jet}=0";
  else if(nj==1)  nj_label = "N_{jet}=1";
  else if(nj==2)  nj_label = "N_{jet}=2";
  else if(nj==3)  nj_label = "N_{jet}=3";
  else if(nj==4)  nj_label = "N_{jet}#geq4";
  else if(nj==5)  nj_label = "N_{jet}#geq0";

...
RootTupleMakerV2_Muons_Extra::RootTupleMakerV2_Muons_Extra(const edm::ParameterSet& iConfig) :
...
void RootTupleMakerV2_Muons_Extra::
...
//  iEvent.getByLabel(inputTagBS, muonsBS);
  ...
  if(muons.isValid()) {
    edm::LogInfo("RootTupleMakerV2_MuonsExtraInfo") << "Total # Muons: " << muons->size();
    ...
    for( std::vector<pat::Muon>::const_iterator it = muons->begin(); it != muons->end(); ++it ) {
      ...
      if(storePFIsolation){
          ...
          if(PfChargedHadronIsolation)
              ...
              edm::LogError("RootTupleMakerV2_MuonsExtraError") << "Error! Can't get the isolation deposit " << "PfChargedHadronIsolation";
          if(PfNeutralHadronIsolation)
              PfNeutralHadronIso->push_back(PfNeutralHadronIsolation->depositWithin(0.3));
          else
              edm::LogError("RootTupleMakerV2_MuonsExtraError") << "Error! Can't get the isolation deposit " << "PfNeutralHadronIsolation";
          if(PFGammaIsolation)
              PFGammaIso->push_back(PFGammaIsolation->depositWithin(0.3));
          else
              edm::LogError("RootTupleMakerV2_MuonsExtraError") << "Error! Can't get the isolation deposit " << "PFGammaIsolation";
...
//          delete PFGammaIsolation;
    ...
    edm::LogError("RootTupleMakerV2_MuonsExtraError") << "Error! Can't get the product " << inputTag;
  }

//  if(muonsWithBS.isValid()) {
//      edm::LogInfo("RootTupleMakerV2_MuonsExtraInfo") << "Total # Muons: " << muonsWithBS->size();
//
...
//
//  if(muonsBS.isValid()) {
//       edm::LogInfo("RootTupleMakerV2_MuonsExtraInfo") << "Total # Muons: " << muonsBS->size();
//
